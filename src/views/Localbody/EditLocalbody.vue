    ...
    <div class="row">
        <div class="col-12 px-0 d-flex flex-wrap">
            ...
            <base-input
                ...
                :disabled="loading"
            ></base-input>
            <base-button :disabled="loading" block @click="upload">Edit Localbody</base-button>
        </div>
...
export default {
    ...
    methods: {
        getLocalbody() {
            this.loading = true;
            const localbodyId = this.localbodyId;

            this.$axios({
                method: 'get',
                url: `/localbodies/localbody/${localbodyId}`,
            }).then((response) => {
                if (response.data && response.data.status === "success") {
                    this.localbody = Object.assign({}, this.localbody, response.data.localbody);
                } else {
                    throw new Error('Localbody Not Found.')
                }
            }).catch(() => {
                this.$error('Localbody Not Found.');
            }).finally(() => {
                this.loading = false;
            });
        },
